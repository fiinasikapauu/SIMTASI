/*
  ...
  - The primary key for the `topikta` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to drop the column `createdAt` on the `topikta` table. All the data in the column will be lost.
  - You are about to drop the column `id` on the `topikta` table. All the data in the column will be lost.
  - Added the required column `id_topikta` to the `topikta` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE `topikta` DROP PRIMARY KEY,
    DROP COLUMN `createdAt`,
    DROP COLUMN `id`,
    ADD COLUMN `id_topikta` INTEGER NOT NULL AUTO_INCREMENT,
    ADD COLUMN `waktu` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
    ADD PRIMARY KEY (`id_topikta`);
